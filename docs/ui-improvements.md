...
- Filters 使用 `defaultValue`，不能编辑后驱动新的 `TripBrief` 或保存偏好。
- 行程分段展开后显示原始 JSON，而不是航班、住宿、逐日活动和餐厅详情卡。
- HITL 事项只有占位文字，没有酒店选择、确认 brief、升级处理等操作。
- `Review plan`、`Saved trips`、`My trips` 和账户入口没有实际行为。
- 地图、时间线、拖拽编辑、保存/恢复行程尚未实现。

## 实施顺序

### P0：让现有页面真正可操作

- [ ] 将 `FiltersPanel` 改为受控表单，覆盖目的地、日期、人数、预算和偏好字段。
- [ ] 在表单提交时生成新的 `TripBrief`，通过现有 chat/orchestrator 入口重新规划。
- [ ] 增加字段校验、提交中状态、接口失败提示和可恢复的错误状态。
...
- [ ] 把 `TripSection` 的原始 JSON 替换为按 section 类型渲染的结构化详情卡。
- [ ] 为详情卡统一补充标题、摘要、价格、来源/新鲜度说明、状态和主要操作。

### P1：完成确认和 review 流程

- [ ] 在 `ChatPanel` 和 `TripPanel` 中渲染 `plan.hitl` checkpoint。
- [ ] 为酒店选择、确认 brief、处理冲突等事项提供明确的按钮和选中态。
- [ ] 将 HITL 操作结果提交到后端，并用返回的最新 `TripPlan` 更新页面。
- [ ] 把 `Review plan` 改为真正的 review 面板，显示待确认项、冲突和预算变化。
- [ ] 增加空状态、无结果状态、重试按钮和网络错误提示。

### P2：补齐产品级导航和保存体验

- [ ] 实现 `Saved trips` / `My trips` 的页面入口或抽屉。
- [ ] 增加保存当前行程、恢复已保存行程和刷新后恢复状态的交互。
- [ ] 增加 Header 的用户/语言入口占位行为，并确保键盘和屏幕阅读器可用。
- [ ] 统一响应式布局，覆盖窄屏、长标题、长摘要和小预算数值等情况。
