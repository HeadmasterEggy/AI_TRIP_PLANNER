...
**最后更新 2026-09-20**，基线 `175a1cf`。§1 已完成，§4 已取消，新增「币种」一节与新发现若干。
...
`shared/src/money.ts` 的 `AUD_PER` 现在是 `CNY 0.21 / USD 1.55 / JPY 0.0105`，带
`TODO(人工核对)` —— **这是编的近似值，不是核对过的汇率**。换成真实数字，并同步 `RATES_AS_OF`。
产品决策是静态汇率，所以查一次冻结即可。
