...
Merge `origin/main` (19a1bff) into `codex/ui-improvements`, preserving 2fd3030 and its P0–P2 work. Keep asynchronous `/api/demo`, local restore precedence, HITL and request cancellation. No database, payment, real booking, or cross-city transport editing.
...
`POST /api/places/search` accepts text/destination. `POST /api/places/details` accepts placeId. `POST /api/trip/preview-edit` accepts current plan, base version, activity operation and WALK/TRANSIT mode; returns candidate plan, differences, routes and conflicts. Validate all input. Never silently fall back to another provider or mode.
