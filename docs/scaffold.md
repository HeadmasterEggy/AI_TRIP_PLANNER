...
`Map`. Booking returns fictional mock data; maps return fixtures by default
(`USE_MOCK_TOOLS=true`) and have real OpenStreetMap and Google adapters when mocks are disabled.
You can build each remaining slice against a real `TripPlan` shape.
...
- **Specialist model routing** is explicit in `packages/agents/src/models.ts`: all five specialists
  (and the supervisor) currently use DeepSeek; MiniMax is configured but not routed. Each path
  accepts an injected generator, validates structured output and falls back deterministically when
  credentials or results fail.
...
- **Chat contract**: `POST /api/chat` takes `ChatRequest` and returns `ChatResponse`
  ...
  (`components/Workspace.tsx`). The route uses LangChain structured extraction with GPT (or
  Anthropic when only that key is configured) and a conservative English/Chinese parser otherwise.
