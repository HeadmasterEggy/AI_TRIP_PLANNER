...
import { assessBudget, costOf, rollUpCost, ESCALATION_OVERRUN_PCT } from "./budget";
import { detectConflicts, runOrchestrator, DEMO_BRIEF } from "./index";
...
describe("accommodation integration with negotiation", () => {
  ...
  it("also escalates an unresolved sub-10% overrun instead of silently accepting it", async () => {
    const plan = await runOrchestrator({ ...DEMO_BRIEF, budgetTotal: 2300 });
    expect(plan.round).toBe(3);
